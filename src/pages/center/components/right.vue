...
<template>
  <div class="pageRIghtBox" ref="comBox">
    ...
    <div class="topBox" ref="topBox">
      ...
      <el-button size="mini" @click="dialogVisible_help = true">帮助</el-button>
    </div>

    <el-tag class="hover" size="mini" :style="positionReturnBtn" @click="returnToCurrentMonth">返回当前月</el-tag>

    <!-- 固定的表头 -->
    <div class="timeBox" ref="timeBox_0" :style="positionHeaderStyle" @scroll="scroll($event, 'timeBox')">
    ...
    <el-dialog title="提示" :visible.sync="isShowDayDetail" width="550px">
      ...
      <div class="comDialogContentBox">
        ...
        <div>
          <span class="hover" @click="look">查看日产量列表</span>
        </div>
      </div>
    </el-dialog>
    ...
    const timer = setInterval(function () {
      if (Object.keys(that.$refs).length) {
        ...
        if (timeBox.clientWidth && topBox.clientHeight) {
          ...
          that.positionReturnBtn = { position: 'absolute', top: `${clientHeight + 1}px`, right: '2px', zIndex: 1000 }
  ...
  computed: {
    ...
    ...mapState({
      rightTableData(state) {
        ...
        /* ----- 延时后：显示悬浮层 ----- */
        const { rightTableData, activeId } = state
        setTimeout(function () {
